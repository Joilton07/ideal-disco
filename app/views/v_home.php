<div class="col-12 mb-4">
    <h4 class="text-muted">veja as últimas novidades selecionadas para você!</h4>
</div>
<div class="col-12 col-sm-6 col-lg-3 mb-3">
    <div class="card p-0 hvr-glow">
        <img src="<?=BASE_URL."/assets/img/guitar.jpg"?>" alt="prod1" class="card-img-top" widht="50px">

        <div class="card-body">
            <p class="card-text">Sumidouro de palheta</p>
            <small>por</small>
            <h5 class="card-title mb-0">R$ 849,99</h5>
            <small>ou 12x de R$ 119,00 com muito juros</small>
            
            <p class="mt-3 float-right"><a href="<?=BASE_URL."/produto/violao"?>" class="btn btn-sm btn-primary">Já quero!</a></p>
        </div>
    </div>
</div>

<div class="col-12 col-sm-6 col-lg-3 mb-3">
    <div class="card p-0 hvr-glow">
        <img src="<?=BASE_URL."/assets/img/smartphone.jpg"?>" alt="prod1" class="card-img-top" widht="50px">

        <div class="card-body">
            <p class="card-text">Smartphone top dos top!</p>
            <small>por</small>
            <h5 class="card-title mb-0">R$ 8999,99</h5>
            <small>ou 12x de R$ 999,99 com muito juros</small>
            
            <p class="mt-3 float-right"><a href="<?=BASE_URL."/produto/smartphone"?>" class="btn btn-sm btn-primary">Já quero!</a></p>
        </div>
    </div>
</div>